...
import com.github.zabetak.calcite.tutorial.DataType.JavaTypeFactoryImplUnique;
import com.github.zabetak.calcite.tutorial.DataType.RelDataTypeFactoryUnique;
import com.github.zabetak.calcite.tutorial.rules.VitekGroupByRule;
...
public class LuceneQueryProcessor {
  ...
  public static <T> Enumerable<T> execute(String sqlQuery, Type processorType)
    ...
    RelDataTypeFactoryUnique typeFactory = new JavaTypeFactoryImplUnique();
    for (TpchTable table : TpchTable.values()) {
      RelDataTypeFactoryUnique.Builder builder = new RelDataTypeFactoryUnique.Builder(typeFactory);
      for (TpchTable.Column column : table.columns) {
        RelDataType type = typeFactory.createJavaType(column.type);
        builder.add(column.name, type.getSqlTypeName()).unique(column.unique).nullable(column.nullable);
      }
    ...
    //planner.addRule(VitekRule.Config.DEFAULT.toRule());
    planner.addRule(VitekGroupByRule.Config.DEFAULT.toRule());
